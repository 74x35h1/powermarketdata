...
class JEPXBidDownloader:
    ...
    def _ensure_tables(self):
        ...
        # Create a sequence for automatic ID generation (DuckDB pre‑0.10 compatibility)
        self.db.execute_query("CREATE SEQUENCE IF NOT EXISTS downloaded_files_id_seq;")

        self.db.execute_query("""
            CREATE TABLE IF NOT EXISTS downloaded_files (
                id BIGINT PRIMARY KEY DEFAULT nextval('downloaded_files_id_seq'),
                dir_name VARCHAR,
                date DATE,
                filename VARCHAR,
                status VARCHAR,
